%%tbl = pl_write_csv_clusters(tbl,settings)
%
%Extracts cluster-level EEG features from HBN data
%
%302 columns: 50 features * 6 clusters + 2 subject and data quality information
%
%Features include: 
...
%
%Clusters are combinations of:
% * saggital axis: left (l), medial (m), right (r) 
% * anterior-posterior axis: frontal (front), parietal (pari)
%
%christian.pfeiffer@uzh.ch
%22.08.2019
%
function tbl = pl_write_csv_clusters(tbl,settings)
  ...
  for row = 1:size(input_table,1)
    ...
    disp(['..adding features type:clusters, row:',num2str(row),', id:',tbl.ID{1}])
    ...
    for en = {'eyesclosed','eyesopen'}
      for fn = {'delta','theta','alpha','beta','gamma'}
          for dn = {'absmean','relmean'}
            ...
            %compose the header
            if row == 1
              header(1,end+1) = {[en{1},'_','fband','_',fn{1},'_',dn{1},'_',cn{1}(~(cn{1}=='_'))]};
            end
      ...
      for fn = {'theta__','lower_1_alpha__','lower_2_alpha__','upper_alpha__','beta__'}        
        for dn = {'absmean','relmean'}
          for cn = {'l_front','m_front','r_front','l_pari','m_pari','r_pari'}
            ...
            %compose the header
            if row == 1
              header(1,end+1) = {[en{1},'_','indfband','_',fn{1}(~(fn{1}=='_')),'_',dn{1},'_',cn{1}(~(cn{1}=='_'))]};
            end
  ...
  writetable(...
    T,...
    [settings.path.results,'resting_eeg_clusters.csv'],...
    'Delimiter',','...
    );
