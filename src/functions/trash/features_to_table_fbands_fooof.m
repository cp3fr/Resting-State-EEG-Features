function tbl = features_to_table_fbands_fooof(eyes, level, inputdata, s)
  ...
  for fn = {'delta','theta','alpha','beta','gamma'}
    for dn = {'absmean','relmean'}
      ...
      ind_f = strcmpi({inputdata.fbands.welch.fbands.name},fn{1});

      %rowval
      rowval = nanmean(inputdata.fbands.welch.fbands(ind_f).(dn{1}));
  ...
  for mn = {'Max','Derivative','Gravity'}
    for dn = {'freq','amplitude'}
      ...
      %rowval
      rowval = inputdata.fbands.welch.alphaPeak.(['alphapeak',dn{1},mn{1}]);
  ...
  for fn = {'theta__','lower_1_alpha__','lower_2_alpha__','upper_alpha__','beta__'}        
    for dn = {'absmean','relmean'}
      ...
      %pointer to frequeny band
      ind_f = strcmpi({inputdata.fbands.welch.Indfbands.name},fn{1});

      %rowval
      rowval = nanmean(inputdata.fbands.welch.Indfbands(ind_f).(dn{1}));
  ...
  for dn = {'aperiodic','peak'}
    for param = 1:2
      ...
      if isfield(inputdata.fooof.welch.fooof.avg,[dn{1},'_','params'])
        vals = inputdata.fooof.welch.fooof.avg.([dn{1},'_','params']);
        if iscell(vals)
          vals=vals{1};
          if isempty(vals)
            vals = [NaN,NaN];
