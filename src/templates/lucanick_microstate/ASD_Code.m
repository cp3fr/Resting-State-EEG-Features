...
close
%directories
win_dir = 'Z:\HBN_Henzen\asd_analysis\';
mac_dir = '/Volumes/methlab-1/HBN_Henzen/asd_analysis/';
if ispc
   cdir = win_dir; 
elseif ismac
   cdir = mac_dir;
...
EHQ_Tot = readtable(append(cdir,'9994_EHQ_20190625.csv'));
AllData = readtable(append(cdir, 'AllData.csv'));
...
opts = detectImportOptions(append(cdir, 'AllDataBig.csv'));
...
%gender
gender = 0;%male changed <= go >=

%righthandedness
rgthanded =-500;% changed from 48 to -500

% Aplying Filter: For example: Up to 5.99 years equals 5 years etc.
indKeep = floor(Mixtbl.Age)>=ageRange(1) & floor(Mixtbl.Age)<=ageRange(2) &...
    Mixtbl.EHQ_Total>=rgthanded & Mixtbl.Sex>=gender & Mixtbl.Release_Number <=6 & Mixtbl.DX_01_Sub_num >=0;
%changed releasnumber from 3 to 6
Mixtbl = Mixtbl(indKeep,:);
Mixtbl = unique(Mixtbl);

for i=1:length(Mixtbl.Patient_ID)
    if Mixtbl.Patient_ID(i)== "NDARYJ389DWX" | Mixtbl.Patient_ID(i)== "NDARGY148EVU"...
            | Mixtbl.Patient_ID(i)== "NDARKE650TFQ"
        Mixtbl(i,:)=[];
    end
end

AllParticipants = Mixtbl;

disp(['subjects of intrest: ',num2str(size(Mixtbl,1))])
%save progress
cd(append(cdir, 'Microstate_Workspace'))
save('asd_filtered_r6.mat')
...
%add microstatesanalysis folder incase pathes were reset
addpath(genpath(append(cdir, '/eeglab/eeglab2019_0/plugins/Microstate-EEGlab-toolbox-master')))
...
%save progress   
cd(append(cdir, 'Microstate_Workspace'))
save('asd_matched_r6.mat')

%% eeglab -> MST (microstates) von hier her weiter machen!
%init
clear all
clc
eeglab
...
% %change folder variable (path) in Allasd for mac
% for i = 1:size(Allasd.folder)
%     name = Allasd.Patient_ID(i);
%     Allasd.folder(i) = cellstr(append('/Volumes/methlab-1/HBN_Henzen/', name));
% end
...
[ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, [1:119] ,'retrieve',119+1,'study',0);%change!!!!!!
...
%save progress
cd(append(cdir, 'Microstate_Workspace'))
save('asd_Workspace_r6.mat', '-v7.3')
...
%directories
win_dir = 'Z:\HBN_Henzen\asd_analysis\';
mac_dir = '/Volumes/methlab/HBN_Henzen/asd_analysis/';
if ispc
   cdir = win_dir; 
elseif ismac
   cdir = mac_dir;
...
% for i = 1:size(Allasd.folder)
%     name = Allasd.Patient_ID(i);
%     Allasd.folder(i) = cellstr(append('/Volumes/methlab/HBN_Henzen/', name));
% end
...
writetable(Allasd)
writetable(Mixtbl)
writetable(AllParticipants)

%save progress
save('finished product_r6.mat')
save('ads_ms_analysis_finished_r6.mat','-v7.3')